...
#         print(X.shape,X_s2.shape)
        output,ordered_idx = model(X, X_s2, vid_lens_batch)
...
def test(device, model,name,epoch, X_s1_val,X_s2_val, y_val, \
    ...
    output,ordered_idx = model(X1, X2, vid_lens_batch)
...
def main():
    ...
    parser.add_argument('--two_stream_views', type=str, default="1,2", help='2stream views \
    to train like 1,2 ; 1,3 ; 2,3 and so on. Please seperate by comma.  \
    1 --> 0deg, 2 --> 30deg, 3 --> 45deg, 4 --> 60deg, 5 --> 90deg')


    parser.add_argument('--save_path', type=str, default='../results/2stream', help='path where \
...
#     num_epoch= 40
    ...
    #                                                  windowsize, lstm_size, model_1stream_path, 10)
    ...
    network = adenet_2stream(device, pretrained_stream1_model_isTrue, \
    ...
    for epoch in range(1,num_epoch+1):
        ...
        #(device, model, optimizer, datagen, epoch, epochsize, s2_train_X, \
    #     s1_train_vidlens, integral_lens)
        train(device, network, optimizer, datagen, epoch, epochsize, s2_train_X,  \
                                                s1_train_vidlens, integral_lens)
    ...
    #   train_acc, train_loss,  predictions_train, true_label_train = test(device, model,"train",epoch, X_train, y_train,  vid_lens_batch_train, mask_train, idxs_train)
        ...
        #(device, model,name,epoch, X_s1_val,X_s2_val, y_val, vid_lens_batch, mask_val, idxs_val)
